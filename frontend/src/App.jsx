...
function App() {
	...
	const getDatosUsuario = async idToken => {
		...
		if (res.data.idRol == 2) getCitastecnico(res.data.idUsuario) // Obtenemos las citas del tecnico
	...
	const getCitastecnico = async idtecnico => {
		try {
			const res = await axios.get(
				'http://localhost:3000/tecnicos/citas/' + idtecnico
			)
			dispatch(SET_CITAS_tecnico(res.data))
		} catch (error) {
			console.log(error)
		}
	}
