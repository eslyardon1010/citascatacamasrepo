...
import CitatecnicoCard from '../components/CitatecnicoCard'
		...
		if (res.isConfirmed) {
			...
			if (respuesta.data.affectedRows > 0) {
				...
				if (usuarioSlice.idRol == 2)
					dispatch(UPDATE_CITAS_tecnico([idCita, { estado: nuevoEstado }]))
			}
		}
	}
						...
						numero={
							usuarioSlice.idRol == 3
								? usuarioSlice.citasClientes?.filter(cita => cita.estado == 2)
										?.length
								: usuarioSlice.citastecnico?.filter(cita => cita.estado == 2)
										?.length
						}
						...
						numero={
							usuarioSlice.idRol == 3
								? usuarioSlice.citasClientes?.filter(cita => cita.estado == 1)
										?.length
								: usuarioSlice.citastecnico?.filter(cita => cita.estado == 1)
										?.length
						}
						...
						numero={
							usuarioSlice.idRol == 3
								? usuarioSlice.citasClientes?.filter(cita => cita.estado == 0)
										?.length
								: usuarioSlice.citastecnico?.filter(cita => cita.estado == 0)
										?.length
						}
						...
						numero={
							usuarioSlice.idRol == 3
								? usuarioSlice.citasClientes?.filter(cita => cita.estado == 3)
										?.length
								: usuarioSlice.citastecnico?.filter(cita => cita.estado == 3)
										?.length
						}
				...
				<div className='w-full h-max flex gap-x-8 justify-between items-start my-6'>
					...
					<div className='w-full flex flex-col justify-start gap-y-5'>
							...
							(usuarioSlice.citasClientes?.length > 0 ? (
								usuarioSlice.citasClientes?.map(cita => (
									<CitaCard
										...
										tecnico={`${cita.nombretecnico} ${cita.ap_paternotecnico}`}
										estado={cita.estado}
						...
						{usuarioSlice.idRol == 2 &&
							(usuarioSlice.citastecnico?.length > 0 ? (
								usuarioSlice.citastecnico?.map(cita => (
									<CitatecnicoCard
										key={cita.idCita}
										cita={cita}
										onClickPendiente={() =>
