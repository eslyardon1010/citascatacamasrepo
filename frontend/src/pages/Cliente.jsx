...
const Cliente = () => {
	...
	// * * * * * * * * * * * * * * * * * * * * * * *	U S E		E F F E C T		* * * * * * * * * * * * * * * * * * * * * * * * *
	// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	// useEffect para cargar los tecnicos y los servicios
	useEffect(() => {
		gettecnicos() // Cargamos los tecnicos de la bd
	...
	// useEffect que actualiza los horarios disponibles cada vez que se agregue / elimine un servicio del carrito o cuando se seleccione otro tecnico
	useEffect(() => {
		if (tecnicoSeleccionado.idUsuario != '' && cita.servicios?.length != 0) {
			getHorariosDisponibles()
		}
	}, [cita, tecnicoSeleccionado, fechaSeleccionada])

	// Actualizar en el estado cita el atributo idtecnico
	useEffect(() => {
		setCita({
			...cita,
			hora: '',
			idtecnico: tecnicoSeleccionado.idUsuario,
	...
	// Obtiene los tecnicos de la bd
	const gettecnicos = async () => {
		try {
			const res = await axios.get('http://localhost:3000/tecnicos')
	...
	const validarInputsVacios = () => {
		if (tecnicoSeleccionado.idUsuario == '') return true
		return false
	}

	// Obtener los horarios disponibles segun el tecnico y la fecha seleccionada
	const getHorariosDisponibles = async () => {
		try {
			...
			const horario = await axios.get(
				`http://localhost:3000/horario/${tecnicoSeleccionado.idUsuario}/${numeroDia}`
			)
			// console.log(horario.data) // me retorna un arreglo
			const citas = await axios.get(
				`http://localhost:3000/citas/${tecnicoSeleccionado.idUsuario}/${fechaSeleccionada}`
			)
			...
			if (horario.data?.length == 0) {
				...
				for (let i = 0; i < horario.data?.length; i++) {
					const horaIniciotecnico = new Date(
						`${fechaSeleccionada}T${horario.data[i].hora_inicio}`
					)
					const horaFintecnico = new Date(
						`${fechaSeleccionada}T${horario.data[i].hora_fin}`
					)
					let horaActual = horaIniciotecnico
					...
					while (
						horaActual < horaFintecnico &&
						horaActualFin < horaFintecnico
					) {
								...
								<div className='w-full'>
									<Select
										...
										opciones={tecnicos.map(tecnico => ({
											...
											texto: `${tecnico.nombre} ${tecnico.ap_paterno} ${tecnico.ap_materno}`,
										}))}
										onChange={e =>
											settecnicoSeleccionado(
												tecnicos.find(
													tecnico => tecnico.idUsuario == e.target.value
												)
											)
										}
										value={tecnicoSeleccionado.idUsuario}
									/>
								</div>
							</div>
							<div>
										...
										<div>
											<div className='flex flex-col w-full gap-y-1'>
												...
												<input
													...
													min={fechaActual}
													disabled={
														tecnicoSeleccionado.idUsuario == '' ? true : false
													}
							...
							{cita.servicios?.length == 0 ? (
								...
								<div className=' bg-[#fcfcfcfa] shadow-sm p-8 overflow-y-auto flex flex-col gap-y-3 text-gray-800'>
									...
									<h4 className='font-semibold'>
										Tecnico:
										<span className='font-light'>{` ${tecnicoSeleccionado.nombre} ${tecnicoSeleccionado.ap_paterno} ${tecnicoSeleccionado.ap_materno}`}</span>
									</h4>
									<h4 className='font-semibold'>
										Fecha:
